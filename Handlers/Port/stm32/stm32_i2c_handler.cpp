...
namespace Handlers
{
	namespace STM32
	{
		...
		I2C::I2C(const I2C_HandleTypeDef* hi2c, const uint32_t address) :
				I2C_Handler(), m_hi2c(hi2c), m_address(address), m_status(I2C_STATUS::NONE)
		{

		}

		I2C::I2C(const I2C& other) : I2C_Handler(), m_status(I2C_STATUS::NONE)
		{
		...
		void I2C::init(const uint32_t trials, const uint32_t timeout)
		{
			if(nullptr != m_hi2c)
			{
		...
		void I2C::send(uint8_t *packet, uint16_t size_packet, uint32_t timeout)
		{
			if(nullptr != m_hi2c)
			{
		...
		I2C_Handler::I2C_STATUS I2C::getStatus() const
		{
			return m_status;
		}

		I2C_Handler::I2C_ERROR I2C::getError() const
		{
			return m_error;
		}

		// private methods
		void I2C::setStatus(const HAL_StatusTypeDef status)
		{
			switch(status)
			{
			case HAL_OK:
		...
		void I2C::setError(const uint32_t error)
		{
			switch(error)
			{
